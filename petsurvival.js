...
// ───── 데이터 로딩 (쟈쟈서버) ─────
let PET_DATA = [];          // 쟈쟈 pet_data.json rows (+ 표시용 필드 부착)
let PET_IMAGES = {};
let selectedPetName = null;

const REPO_JSON = 'pet_data.json';
const API_URL   = 'https://sa000001.com/sa2/info/api/pet_data.php';
const IMG_BASE  = 'https://sa000001.com/sa2/info/img/';
const IMG_ALT   = 'https://saworld.net/PetAnimation/';

/* 속성 상성: 지 → 수 → 화 → 풍 → 지 (쟈쟈로그와 동일) */
const ATTR_STRONG = { '지': '수', '수': '화', '화': '풍', '풍': '지' };
const ELEM_KEY = { earth: '지', water: '수', fire: '화', wind: '풍' };
...
async function loadGameData() {
  let rows = null;
  try {
    const r = await fetch(REPO_JSON + '?t=' + Date.now());
    ...
    try {
      ...
      console.error('펫 데이터 로드 실패:', e2);
      return;
    }
  }
  PET_DATA = rows
    .filter(p => p && p.imageCode && num(p.h0) > 0)
    .map(p => {
      const name = String(p.name || '').replace(/^\uFEFF/, '');
      // 엔진이 기대하는 한글 속성 객체로 변환 (0~100 → 0~10)
      const attr = {};
      for (const k in ELEM_KEY) attr[ELEM_KEY[k]] = num(p[k]) / 10;
      const image = petImgUrl(p.imageCode);
      PET_IMAGES[name] = image;
      return Object.assign({}, p, { name, attr, image, grade: RARITY_KR[String(p.rarity)] || '' });
    });
  console.log('\u2705 ' + PET_DATA.length + ' 마리 펫 로드 (쟈쟈)');
}

// ───── 무기 정의 (이미지 + 동작) ─────
const WEAPON_DEFS = {
  axe: {
    name: '테라죠의 도끼', icon: 'https://sa000001.com/sa2/info/img/item/52058.png',
    desc: '회전하며 주변 적 베기',
  ...
  spear: {
    name: '헤티아의 창', icon: 'https://sa000001.com/sa2/info/img/item/36281.png',
    desc: '직선 길게 찌르기',
  ...
  claw: {
    name: '흑룡의 발톱', icon: 'https://sa000001.com/sa2/info/img/item/36283.png',
    desc: '빠른 연속 할퀴기',
  ...
  bow: {
    name: '정령왕의 활', icon: 'https://sa000001.com/sa2/info/img/item/20325.png',
    desc: '적을 자동 추적하는 화살',
  ...
  hammer: {
    name: '의식의 곤봉', icon: 'https://sa000001.com/sa2/info/img/item/36476.png',
    desc: '앞 방향 부채꼴 휘두르기',
    cooldown: 1300, damage: 1.8, range: 180,
    style: 'sweep',
  },
  ice: {
    name: '얼음 손톱', icon: 'https://sa000001.com/sa2/info/img/item/36289.png',
    desc: '눈꽃 발사 (얼리고 슬로우)',
    cooldown: 1500, damage: 1.2, range: 400,
    style: 'freeze',
  },
  stone: {
    name: '합성 돌9', icon: 'https://sa000001.com/sa2/info/img/item/36186.png',
    desc: '하늘에서 떨어지는 돌',
...
function renderPetGrid(filter = '') {
  ...
  grid.innerHTML = filtered.slice(0, 100).map(p => {
    ...
    let mainA = '', mainV = 0;
    for (const a of ['지','수','화','풍']) if ((p.attr[a]||0) > mainV) { mainV = p.attr[a]; mainA = a; }
    const grade = p.grade ? `<b>${p.grade}</b> ` : '';
    return `
      <div class="pet-card" data-name="${p.name}" onclick="selectPet('${p.name.replace(/'/g,"\\'")}')">
        ...
        <div class="pet-card-stats">${grade}${mainA}${Math.round(mainV*10)}</div>
      </div>
    `;
  }).join('');
}
...
// ───── 펫 능력치 (쟈쟈 정식 공식) ─────
/* simulator.html 과 동일:
   disp(h,a,d,s) = 성장베이스 → 표시 능력치(내구/공격/방어/순발)
   초기치   = disp(initPct * (기본치+lvup) / 100)
   레벨당성장 = disp((기본치+lvup) * B / 10000),  B = rankInfoB(기본치 합) */
function rankInfoB(sum){
  if(sum>=100) return 475; if(sum>=95) return 495; if(sum>=90) return 515;
  if(sum>=85)  return 535; if(sum>=80) return 555; return 575;
}
function disp(h,a,d,s){
  return {
    h: h*4 + a + d + s,
    a: h*0.1 + a + d*0.1 + s*0.05,
    d: h*0.1 + a*0.1 + d + s*0.05,
    s: s
  };
}
/* roll: 성장베이스 보정 -2~+2 + 배분치 10포인트. 전부 +2 = 8등급(환각) */
function spread10(){
  const o = { hb:0, ab:0, db:0, sb:0 }, k = ['hb','ab','db','sb'];
  for(let i=0;i<10;i++) o[k[Math.floor(Math.random()*4)]]++;
  return o;
}
function rollInit(hallu){
  if(hallu) return Object.assign({ rh:2, ra:2, rd:2, rs:2 }, spread10());
  const r = () => Math.floor(Math.random()*5) - 2;
  return Object.assign({ rh:r(), ra:r(), rd:r(), rs:r() }, spread10());
}
function rollGrade(roll){
  const score = (roll.rh+2)+(roll.ra+2)+(roll.rd+2)+(roll.rs+2);
  if(score === 16) return 8;
  if(score === 0)  return 1;
  return Math.max(2, Math.min(7, 2 + Math.round((score - 1) * 5 / 14)));
}
/* 140레벨 기준 능력치 */
function jajaStats(info, roll, level){
  level = level || 140;
  ...
  return {
    hp:  Math.max(1, Math.round(init.h + growth.h * L)),
    atk: Math.max(1, Math.round(init.a + growth.a * L)),
    def: Math.max(1, Math.round(init.d + growth.d * L)),
    agi: Math.max(1, Math.round(init.s + growth.s * L)),
  };
}

function getPetStats(name, roll) {
  const info = PET_DATA.find(p => p.name === name);
  if (!info) return null;
  roll = roll || rollInit(false);
  const st = jajaStats(info, roll, 140);
  ...
  return {
    ...
    attr, mainAttr, roll, grade: rollGrade(roll),
    baseAtk: st.atk, baseDef: st.def,
    baseSpd: st.agi, baseHp: st.hp,
  };
}
...
function createPetDOM(name, imgUrl, isPlayer, isBoss, bossLabel) {
  ...
  if (imgUrl) {
    ...
    el.innerHTML = `<img src="${imgUrl}" alt="${name}" style="width:100%;height:100%;object-fit:contain;image-rendering:pixelated;filter:${dropShadow};"`
      + ` onerror="this.onerror=null;this.src='${IMG_ALT}' + this.src.split('/').pop().replace('.gif','') + '.gif';">`;
  } else {
...
async function startGame() {
  ...
  G.player = {
    ...
    grade: stats.grade, roll: stats.roll,
    radius: 30,
  ...
  setTimeout(() => {
    const g = stats.grade;
    const msg = g === 8 ? '\u2605 8등급 환각! \u2605' : g + '등급 개체';
    addFloatingText(G.player.x, G.player.y - 70, msg,
      g === 8 ? '#f5a623' : (g >= 6 ? '#6fd0a0' : '#ffffff'), g >= 6);
  }, 400);
}
...
function updateEnemies(dt, now) {
  ...
  for (let i = G.enemies.length - 1; i >= 0; i--) {
    ...
    if (dist < p.radius + e.radius && now > p.invincibleUntil) {
      ...
      const dmg = Math.max(1, Math.round((e.atk - (p.def * DEF_COEF + G.passives.defAdd)) * mult));
      p.hp -= dmg;
...
function spawnEnemy() {
  ...
  const enemyPets = PET_DATA.filter(p => String(p.rarity) === '0');
  ...
  const e = {
    ...
    hp: Math.round(40 * lvMul * ENEMY_HP_SCALE), maxHp: Math.round(40 * lvMul * ENEMY_HP_SCALE),
    atk: Math.round(80 * lvMul * ENEMY_ATK_SCALE),
...
// 보스 = 환각(8등급) 개체. 쟈쟈엔 랭커 데이터가 없으므로 초기치 굴림으로 대체한다.
function spawnBoss() {
  const angle = Math.random() * Math.PI * 2;
  const dist = Math.max(G.W, G.H) * 0.7;
  const x = G.player.x + Math.cos(angle) * dist;
  const y = G.player.y + Math.sin(angle) * dist;
  // 성장베이스 합이 높은 펫 중에서 (돈펫 포함 — 보스니까)
  const pool = PET_DATA.slice().sort((a, b) =>
    (num(b.h0)+num(b.a0)+num(b.d0)+num(b.s0)) - (num(a.h0)+num(a.a0)+num(a.d0)+num(a.s0))
  ).slice(0, 60);
  const ep = pool[Math.floor(Math.random() * pool.length)] || PET_DATA[0];
  // 3분 지나면 진짜 환각(전부 +2), 그 전엔 준수한 개체
  const hallu = G.time >= 180;
  const roll  = hallu ? rollInit(true) : Object.assign({ rh:1, ra:1, rd:1, rs:1 }, spread10());
  const grade = rollGrade(roll);
  const bossLabel = (grade === 8 ? '환각 ' : '우두머리 ') + ep.name;
  const e = {
    x, y, name: ep.name, attr: ep.attr, radius: 50,
    hp: Math.round(500 * G.difficultyMul * ENEMY_HP_SCALE),
    maxHp: Math.round(500 * G.difficultyMul * ENEMY_HP_SCALE),
    atk: Math.round(120 * G.difficultyMul * ENEMY_ATK_SCALE),
    speed: 70, boss: true, frozenUntil: 0,
    grade, bossLabel,
    dom: createPetDOM(ep.name, ep.image, false, true, bossLabel),
  };
  G.enemies.push(e);
  addFloatingText(x, y - 60, '\u26a0\ufe0f ' + bossLabel + ' 등장!', grade === 8 ? '#f5a623' : '#ff5252', true);
}
...
function gameOver() {
  ...
  document.getElementById('gameover-stats').innerHTML = `
    ...
    <div class="gameover-stat-row"><span class="gameover-stat-label">🐾 펫</span>
      <span class="gameover-stat-val">${G.player.name} <small>(${G.player.grade}등급)</small></span></div>
  `;
